...
Why: the project runs on a Gemini API key. The two-phase research design carries over unchanged behind the same `SearchProvider` protocol — phase one grounds a research pass in Google Search, phase two extracts typed findings with a `response_schema`-constrained call (Gemini cannot combine search grounding with JSON mode in one request, so the split is now load-bearing, not just tidy). One Gemini-specific addition: grounded citations arrive as expiring Google redirect URLs in metadata, not as text, so the provider inserts citation markers from `grounding_supports` and resolves each redirect (a single non-following HEAD to Google) into a verified source list that extraction copies URLs from — provenance comes from search metadata, never model memory. Default model `gemini-3.6-flash` (newest stable Flash; the 2.5 line is retired for new accounts, and the `gemini-flash-latest` alias proved quota-less on a fresh key), overridable via `MOTO_RESEARCH_MODEL`. Caveat discovered on the first live run: Google Search grounding has no free-tier quota on new keys — research needs a billing-enabled Gemini key, while everything else (including JSON-schema extraction) works without it. Chat, when built, uses `langchain-google-genai`.
...
**2026-07-22 — Manufacturer roster and official-tier domains share one source of truth: `MANUFACTURER_OFFICIAL_DOMAINS` in the catalog registry.**
Why: the official-domain allowlist in research's tiering and the seeded manufacturer list had drifted (19 brands trusted as official sources vs. 6 manufacturers browsable in the catalog). Keyed by manufacturer name in catalog — identity is catalog's domain — the roster now feeds both the manufacturer seed and the derived `OFFICIAL_DOMAINS` frozenset in `tiering.py`, so trusting a brand's official site and listing that brand in the catalog are the same edit. Tested-tier domains stay a plain set in `tiering.py`; they have no catalog counterpart.

**2026-07-22 — Chat agent: hand-rolled two-node LangGraph loop; structured blocks ride tool artifacts; status lines derive from tool calls.**
Why: a custom `agent ⇄ tools` StateGraph (over prebuilt `create_react_agent`) keeps streaming and tool execution fully in our hands. Tools return `(content, blocks)` via LangChain's `content_and_artifact`: the LLM reads a compact text rendering while the user receives the full typed payload from the same execution — structured data never round-trips through the model, so it can't be mangled or fabricated. `status` events map deterministically from tool calls, prose streams token-by-token, and blocks are emitted as tool artifacts when each tool node completes. Tool calls run sequentially — the toolbox shares the request's SQLAlchemy session, which is not thread-safe — and expected service errors return as tool content so the model can self-correct, while unexpected exceptions fail the stream as an `error` event instead of feeding the model garbage.

**2026-07-22 — Conversation persistence: in-memory LangGraph checkpointer, one thread per conversation_id. (Resolves chat.md's open TODO.)**
Why: chat history is not part of the product's accumulating asset — the bike database is — so v1 doesn't earn a conversations table. `InMemorySaver` gives real multi-turn memory for the process lifetime; if durable conversations are ever wanted, a Postgres-backed checkpointer swaps in at graph compile time with no graph changes. The conversation id is client-supplied or server-generated on the first message and returned in the `done` event.

**2026-07-22 — Chat degrades to 503 without a Gemini key; the model is built at startup, not import.**
Why: a keyless checkout (CI, fresh clone) must still boot, test, and serve catalog/profile/research. `ChatGoogleGenerativeAI` refuses to construct without credentials, so `create_app` builds it only when a key resolves and the chat endpoint otherwise answers 503 naming `MOTO_GEMINI_API_KEY` — the same graceful-degradation stance as research's billing-key caveat.
